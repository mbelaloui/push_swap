...
/*   Updated: 2018/06/06 08:12:01 by mbelalou         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
BOOL		ft_add_end_values_int_list(char *line, t_int_list **list_val,
		t_options *option)
{
	int			pt;
	intmax_t	val;
	...
	while (line[pt])
	{
		...
		if (line[pt] == '-' || line[pt] == '+')
			pt++;
		if (ft_is_in_int_list(val, *list_val))
			return (ft_clear_intlist(list_val)
					&& (option->exist_double = 1) && F);
		else if (!ft_add_end_intlist(val, list_val))
			return (ft_clear_intlist(list_val) & F);
		size_nbr = 0;
		while (line[pt] && ft_isdigit(line[pt++]))
			size_nbr++;
		if (is_overlow(line + pt - size_nbr - 1, val, *option))
			return (ft_clear_intlist(list_val) && (option->overflow = 1) && F);
	}
	return (T);
}
