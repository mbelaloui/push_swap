...
/*   Updated: 2018/05/31 18:41:25 by mbelalou         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
		...
		t_charlist **action)
{
	...
	sizelist_b = ft_size_intlist(*list_a);
	...
	while (sizelist > 2 && !ft_is_sorted_asc_int_list(*list_a))
	{
		...
		if ((*list_a)->data <= (intmax_t)med && (temp > (*list_a)->data))
		{
			...
			ft_max_int_list(*list_b, &temp);//->data;
			temp = temp + 20;
		}
		else
		{
...
void	final_pahse(t_int_list **list_a, t_options *option, t_charlist *action)
{
	...
	if (index <= ft_size_intlist(*list_a) / 2)
		...
		ft_rotat_down_stack_a(list_a, ft_size_intlist(*list_a) - index, &action);
}

void	first_phase_test(t_int_list **list_a, t_int_list **list_b, t_options *option,
		t_charlist **action)
{
	int			sizelist;
	long double	med;
	...
	ft_get_pivot(*list_a, sizelist, &pivot, pres);
	while (sizelist > 1 && !ft_is_sorted_asc_int_list(*list_a))
	{
...
//	ft_min_int_list(*list_a, &min);//->data;
	//		ft_printf(" pivot = %ld \t min = %ld \n", pivot, min);
	//		if (pivot == min)
	/*ft_printf( "pivot  = %d\n",*/// pivot = get_pivot(*list_a, sizelist);
	//break;
}
...
void	run_sort(t_int_list *list_a, t_options *option)
{
	...
	action = NULL;
	list_b = NULL;
	first_phase_test(&list_a,&list_b, option, &action);
