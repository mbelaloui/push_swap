...
/*   Updated: 2018/05/18 12:42:08 by mbelalou         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static BOOL	is_overlow(char *line, intmax_t val, int size_nbr, t_options option)
{
	if (option.l)
		...
		t_options *option)
{
	...
	while (line[pt])
	{
		...
		while (line[pt] && ft_isdigit(line[pt++]))
			size_nbr++;
		if (is_overlow(line + pt - size_nbr - 1, val, size_nbr, *option))
